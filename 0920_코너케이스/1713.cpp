...
bool cmp(const pair<int, ci> &c1, const pair<int, ci> &c2) //정렬 함수
{
    if (c1.second.first != c2.second.first)
    {                                             // 추천 횟수가 다를 경우
        return c1.second.first < c2.second.first; //추천횟순으로 오름차순
    }
    return c1.second.second < c2.second.second; //개시 시간대로 오름차순
}

int main()
{
    int n, m, input; // n : 사진틀 개수, k: 총 추천 횟수, input: 추천받은 학생의 번호
    // 입력
    cin >> n >> m;
    ...
    for (int i = 0; i < m; i++)
    {
        cin >> input; // m번 만큼 추천 받음
        //비어있는 사진틀이 없는 경우
        if (candidate.size() == n && candidate.find(input) == candidate.end())
        {
            candidate.erase(min_element(candidate.begin(), candidate.end(), cmp)); // 후보 삭제, 가작 적은 요소를 가진 element를 삭제한다 = 추천수가 가장 적은 후보 삭제
        }
        //후보 첫 게시
